<template>
  <div class="flex items-center justify-between w-full mt-2 text-sm">
    <label v-if="tax.calculation_type === 'percentage'" class="font-semibold leading-5 text-gray-500 uppercase dark:text-gray-400">
      {{ tax.name }} ({{ tax.percent }} %)
    </label>
    <label v-else class="font-semibold leading-5 text-gray-500 uppercase dark:text-gray-400">
      {{ tax.name }} (<BaseFormatMoney :amount="tax.fixed_amount" :currency="currency" />)
    </label>
    <label class="flex items-center justify-center text-lg text-black dark:text-gray-100">
